...
class KF_Sim:
    ...
    def __init__(self, truth_sim: Sim, dx0: np.ndarray, dx_est0: np.ndarray,
        ...
        self.dx_est, self.dy_est, self.err_cov, self.inn_cov = (
            linearized_ekf_sim.linrun_linearized_kf(self.truth.x_nom,
                                                    self.truth.y_true,
...
def run_monte_carlo(tfinal: float, x0_true: np.ndarray,
                    num_sims: int) -> List[KF_Sim]:
