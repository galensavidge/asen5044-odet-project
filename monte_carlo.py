...
class KF_Sim:
    ...
    def __init__(self, nom_sim: Sim, x_est0: np.ndarray, P0: np.ndarray,
        ...
        Args:
            ...
            kf_type: 'LKF' will run the linearized KF (above inputs are then
                related to perturbations from the nominal trajectory). 'EKF'
                will run the extended KF (above inputs are related to the full
                state)
        """

        if kf_type != 'LKF' and kf_type != 'EKF':
            raise ValueError(
                f'KF type {kf_type} is not valid. Use LKF or EKF.')
    ...
    def integrate_nom_traj(self, x_true0: np.ndarray):
        ...
        proccess_noise = problem_setup.form_process_noise(
            self.nom.T, self.nom.op.W)
        ...
        self.y_nom = problem_setup.states_to_meas(self.nom.x_nom,
                                                  self.nom.time,
                                                  self.station_ids_list)
        self.y_true = problem_setup.states_to_noisy_meas(
            self.x_true, self.nom.time, self.station_ids_list, self.nom.op.R)

    def linearized_kalman_filter(self, dx_est0: np.ndarray, P0: np.ndarray,
                                 Qkf: np.ndarray, Rkf: np.ndarray):

        # find nonlinear perturbations
        self.dx = self.x_true - self.nom.x_nom
        self.dy = problem_setup.addsubtract_meas_vecs(self.y_true, self.y_nom,
                                                      -1)

        # Estimated filter state and measurements
        self.dx_est, self.dy_est, self.err_cov, self.inn_cov = (
            linearized_ekf_sim.run_linearized_kf(self.nom.x_nom, self.y_true,
                                                 self.nom.time, dx_est0, P0,
                                                 self.nom.op.dt, Qkf, Rkf))
        self.x_est = self.dx_est + self.nom.x_nom
        self.y_est = problem_setup.addsubtract_meas_vecs(
            self.dy_est, self.y_nom, 1)

        # Find error
        self.dx_err = self.dx - self.dx_est
        self.dy_err = problem_setup.addsubtract_meas_vecs(
            self.dy, self.dy_est, -1)
...
def run_monte_carlo(tfinal: float, x0_nom: np.ndarray, x0_est: np.ndarray,
                    P0: np.ndarray, Q: np.ndarray, R: np.ndarray,
                    num_sims: int, kf_type: str) -> List[KF_Sim]:
