...
def example_sim_wnoise():
    ...
    w = problem_setup.form_process_noise(int(np.floor(op.T0/op.dt)),process_cov)
    t, x_k = integrate_nl_ct_eom(op.x0,op.dt,op.T0,w)
...
def nonlinear_ct_eom(t:float,x: np.ndarray,w:np.ndarray,dt:float) -> np.ndarray:
...
def integrate_nl_ct_eom(x0: np.ndarray, dt: float,
                        t_final: float, w: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
