...
def measurements(y_k: np.ndarray,t: np.ndarray,axs: list[matplotlib.axes.Axes],legend_label:str = ''):
    ...
    # 3d array: 1st dim is station id, second is time step, third is meas vector
    ...
    for st_id in range(12):
        axs[0].plot(t,y_id[st_id,:,0],label=legend_label,color=id_colors[st_id])
        axs[1].plot(t,y_id[st_id,:,1],label=legend_label,color=id_colors[st_id])
        axs[2].plot(t,y_id[st_id,:,2],label=legend_label,color=id_colors[st_id])
        axs[3].plot(t,t_id[st_id,:],label=legend_label,color=id_colors[st_id])
